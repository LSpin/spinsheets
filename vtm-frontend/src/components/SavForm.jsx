    ...
    return (
      <div className="blades-action-column">
        ...
        <div className="blades-xp-row mt-sm" >
          <span className="blades-action-label text-sm" >{title} XP</span>
  ...
  return (
    <div className={viewMode ? 'form-view-mode' : ''}>
      ...
      <div className="flex items-center gap-sm mb-sm" >
        <label className="flex items-center text-sm cursor-pointer" style={{ gap: '0.4rem' }}>
          ...
          <fieldset>
            ...
            {selectedPlaybook && (
              <div className="form-section p-md mt-sm" style={{ background: 'rgba(52,152,219,0.08)', borderLeft: '3px solid var(--color-accent-fg)' }}>
                ...
                {selectedPlaybook.supernatural && <span className="splat-badge splat-badge--sav text-sm mb-xs" style={{ display: 'inline-block' }}>Deep Cuts — Supernatural</span>}
      ...
      <div hidden={tab !== 'tabSavActions'} role="tabpanel" aria-labelledby="tabBladesActions">
        <div className="form-section">
          {/* Gambit Pool */}
          <div className="flex items-center justify-center gap-md mb-md p-md bg-surface rounded border" >
            <span className="font-bold text-md uppercase text-accent" style={{ letterSpacing: '0.05em' }}>{t('savGambits')}</span>
            <button type="button" className="btn btn-secondary font-bold" style={{ minWidth: 36, padding: '0.2rem 0.5rem' }}
              onClick={() => setGambits(Math.max(0, gambits - 1))} disabled={gambits <= 0}>-</button>
            <span className="font-bold text-center" style={{ fontSize: '1.8rem', minWidth: 40 }}>{gambits}</span>
            <button type="button" className="btn btn-secondary font-bold" style={{ minWidth: 36, padding: '0.2rem 0.5rem' }}
          ...
          {selectedPlaybook?.xpTrigger && (
            <div className="mb-md rounded text-sm" style={{ padding: 'var(--space-sm) var(--space-md)', background: 'rgba(243,156,18,0.08)', border: '1px solid rgba(243,156,18,0.25)' }}>
        ...
        <div className="form-section">
          <fieldset>
            ...
            {selectedPlaybook?.abilities?.map(ability => (
              <label key={ability.name} className="blades-ability-row flex gap-sm items-start" style={{ padding: 'var(--space-xs) 0' }}>
        ...
        <div className="form-section">
          ...
          <fieldset>
            ...
            <div className="flex flex-wrap gap-sm" >
              {BLADES_TRAUMA_CONDITIONS.map(tc => (
                <label key={tc} className="flex items-center text-base" style={{ gap: '4px' }}>
          ...
          <fieldset>
            ...
            <div className="flex-col gap-sm" >
              <div className="field-row">
                <span className="font-semibold text-base" style={{ minWidth: '80px' }}>{t('bladesHarmLevel3')}</span>
                <div className="field flex-1" ><input name="bladesHarm3" value={fields.bladesHarm3} onChange={handleText} placeholder="Need fatal" /></div>
              </div>
              <div className="field-row">
                <span className="font-semibold text-base" style={{ minWidth: '80px' }}>{t('bladesHarmLevel2')}</span>
                <div className="field flex-1" ><input name="bladesHarm2a" value={fields.bladesHarm2a} onChange={handleText} placeholder="-1d" /></div>
                <div className="field flex-1" ><input name="bladesHarm2b" value={fields.bladesHarm2b} onChange={handleText} placeholder="-1d" /></div>
              </div>
              <div className="field-row">
                <span className="font-semibold text-base" style={{ minWidth: '80px' }}>{t('bladesHarmLevel1')}</span>
                <div className="field flex-1" ><input name="bladesHarm1a" value={fields.bladesHarm1a} onChange={handleText} placeholder="Less effect" /></div>
                <div className="field flex-1" ><input name="bladesHarm1b" value={fields.bladesHarm1b} onChange={handleText} placeholder="Less effect" /></div>
              </div>
            </div>
          </fieldset>
          <fieldset>
          ...
          <fieldset>
            ...
            <div className="flex gap-md items-center mb-sm" >
              {LOAD_OPTIONS.map(opt => (
                <label key={opt.value} className="flex items-center text-base" style={{ gap: '4px' }}>
      ...
      <div hidden={tab !== 'tabBladesProjects'} role="tabpanel" aria-labelledby="tabBladesProjects">
        <div className="form-section">
              ...
              {clocks.map((clock, ci) => (
                <div key={ci} role="listitem" className="p-sm border" style={{ borderRadius: '8px', background: 'rgba(52,152,219,0.04)' }}>
                  <div className="flex justify-between items-center mb-xs" >
                    ...
                    <span className="text-sm text-muted" >{clock.filled}/{clock.total}</span>
                  </div>
                      ...
                      {Array.from({ length: clock.total }, (_, si) => {
                        ...
                        return (
                          <path key={si} d={d}
                            ...
                            className="cursor-pointer" style={{ opacity: si < clock.filled ? 0.85 : 0.3 }}
                            onClick={() => {
                              const next = [...clocks]
          ...
          <details className="mt-sm" >
            ...
            <div className="grid gap-sm mt-sm" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
              {[
                ...
                <div key={act.key} className="p-sm border rounded bg-surface" >
                  ...
                  <div className="text-sm text-muted lh-snug" >{t(act.descKey)}</div>
                </div>
              ))}
            </div>
          </details>
                ...
                <div className="flex" style={{ gap: '4px' }}>
                  {[1, 2, 3, 4].map(i => (
                    <span key={i} role="button" tabIndex={0}
                      ...
                      className="flex items-center justify-center cursor-pointer text-sm font-bold" style={{ width: 28, height: 28, borderRadius: '4px', border: '2px solid var(--color-border)' }}
              ...
              <div className="field flex-1" >
                ...
                <div className="flex flex-wrap" style={{ gap: '2px' }}>
                  {Array.from({ length: 40 }, (_, i) => (
                    <span key={i} role="button" tabIndex={0}
                      className="cursor-pointer border" style={{ width: 14, height: 14, borderRadius: '2px', background: i < (fields.bladesStash || 0) ? 'var(--color-accent-fg)' : 'var(--color-surface-raised)', opacity: i < (fields.bladesStash || 0) ? 1 : 0.5 }}
                ...
                {(fields.bladesStash || 0) >= 40 && (
                  <p className="text-accent font-bold text-sm mt-xs" >
                    {t('bladesStashRetire')}
                  </p>
                )}
              </div>
        ...
        <div className="form-section">
          {selectedPlaybook?.contacts?.length > 0 && (
            <fieldset>
              ...
              {selectedPlaybook.contacts.map(contact => {
                ...
                return (
                  <div key={contact} className="flex items-center gap-sm text-md" style={{ padding: 'var(--space-xs) 0' }}>
                    <button type="button" className={`btn btn-secondary${rel === '+' ? ' tab-btn--active' : ''} text-base`}
          ...
          <fieldset>
            ...
            {contacts.filter(c => !selectedPlaybook?.contacts?.includes(c.name)).map(c => (
              <div key={c.name} className="flex items-center gap-sm text-md" style={{ padding: 'var(--space-xs) 0' }}>
                <button type="button" className={`btn btn-secondary${c.rel === '+' ? ' tab-btn--active' : ''} text-base`}
                  style={{ minWidth: '32px', padding: '2px 8px' }}
                ...
                <button type="button" className="btn btn-danger text-sm" style={{ padding: '2px 6px', marginLeft: 'auto' }}
                  onClick={() => {
      ...
      <div hidden={tab !== 'tabDiceRoller'} role="tabpanel" aria-labelledby="tabDiceRoller">
        <details className="mb-md p-sm border" style={{ borderRadius: '8px', background: 'rgba(243,156,18,0.06)' }}>
          ...
          <div role="note" aria-live="polite" className="text-base lh-normal text-muted" style={{ padding: 'var(--space-sm) 0' }}>
