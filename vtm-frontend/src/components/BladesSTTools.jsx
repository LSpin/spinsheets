...
const ST_CLOCKS_KEY = 'blades-st-clocks'
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-sm)', flexWrap: 'wrap' }}>
        ...
        {result && (
          <>
            <button className="btn btn-secondary" onClick={() => rollSingle('clients')}>{t('bladesSTRerollClient')}</button>
            <button className="btn btn-secondary" onClick={() => rollSingle('targets')}>{t('bladesSTRerollTarget')}</button>
            <button className="btn btn-secondary" onClick={() => rollSingle('workTypes')}>{t('bladesSTRerollWork')}</button>
            <button className="btn btn-secondary" onClick={() => rollSingle('complications')}>{t('bladesSTRerollComplication')}</button>
            <button className="btn btn-secondary" onClick={() => rollSingle('twists')}>{t('bladesSTRerollTwist')}</button>
          </>
        )}
      </div>
      ...
      {result && (
        <ResultCard>
          <div style={{ display: 'grid', gap: 'var(--space-sm)' }}>
            ...
            <div><strong>{t('bladesSTComplication')}:</strong> {result.complication}</div>
            <div><strong>{t('bladesSTTwist')}:</strong> {result.twist}</div>
          </div>
        </ResultCard>
      )}
  ...
  return (
    <div>
      ...
      <button className="btn btn-primary" onClick={rollEvent}>
        {result ? t('bladesSTRollAnother') : t('bladesSTRollEvent')}
      </button>
  ...
  return (
    <div>
      ...
      <button className="btn btn-primary" onClick={rollBargain}>
        {result ? t('bladesSTAnotherBargain') : t('bladesSTRollBargain')}
      </button>
    ...
    <div>
      ...
      {clocks.length === 0 && (
        <div className="empty-state" style={{ padding: 'var(--space-2xl)' }}>
          ...
          <p className="muted-hint">{t('bladesSTClocksEmpty')}</p>
        </div>
      )}
...
const TABS = [
  { key: 'score', label: 'bladesSTTabScore' },
  { key: 'entanglement', label: 'bladesSTTabEntanglement' },
  { key: 'npc', label: 'bladesSTTabNPC' },
  { key: 'faction', label: 'bladesSTTabFaction' },
  { key: 'downtime', label: 'bladesSTTabDowntime' },
  { key: 'bargain', label: 'bladesSTTabBargain' },
  { key: 'clocks', label: 'bladesSTTabClocks' },
]

export default function BladesSTTools() {
